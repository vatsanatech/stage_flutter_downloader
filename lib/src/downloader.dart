...
class FlutterDownloader {
  ...
  /// Creates an Apple-managed offline HLS download task.
  ///
  /// This method is intentionally separate from [enqueue] so existing file
  /// downloads and the Android implementation remain unchanged. The returned
  /// identifier works with [pause], [resume], [cancel], [remove], and
  /// [loadTasks].
  static Future<String?> enqueueHls({
    required String url,
    ...
    assert(_initialized, 'plugin flutter_downloader is not initialized');
    if (!Platform.isIOS) {
      throw UnsupportedError('enqueueHls is currently supported only on iOS');
    }

    try {
      final taskId = await _channel.invokeMethod<String>('enqueueHls', {
        'url': url,
      ...
      if (taskId == null) {
        throw const FlutterDownloaderException(
          message: '`enqueueHls` returned null taskId',
        );
      }
      return taskId;
    } on FlutterDownloaderException catch (err) {
      _log('Failed to enqueue HLS. Reason: ${err.message}');
    } on PlatformException catch (err) {
      _log('Failed to enqueue HLS. Reason: ${err.message}');
    }
    return null;
  ...
  /// Returns whether an Apple-managed HLS task has a valid asset that can play
  /// without network access.
  static Future<bool> isPlayableOffline({required String taskId}) async {
    assert(_initialized, 'plugin flutter_downloader is not initialized');
    if (!Platform.isIOS) return false;
    return await _channel.invokeMethod<bool>('isPlayableOffline', {
          'task_id': taskId,
        }) ??
        false;
  }
    ...
    try {
      ...
      return result.map((dynamic item) {
        return DownloadTask(
          ...
          localFilePath: item['local_file_path'] as String?,
        );
      }).toList();
    } on FlutterDownloaderException catch (err) {
